...
func TestAccAWSAutoScalingGroup_InstanceRefresh_Triggers(t *testing.T) {
	checkInstanceRefreshTokenNotEquals := func(resourceName string, lastToken *string) resource.TestCheckFunc {
		return func(state *terraform.State) error {
			...
			refreshToken := rs.Primary.Attributes["instance_refresh_token"]
			if refreshToken == "" {
				return fmt.Errorf("instance_refresh_token is empty")
			}

			if refreshToken == *lastToken {
				return fmt.Errorf("expected the refresh token to change")
			}
	...
	matrix := []struct {
		...
		{2, 0, "t3.nano", true, false, false, false, false},  // create asg with 2 az-s
		{2, 0, "t3.nano", true, false, false, false, true},   // add propagated tag
		{2, 0, "t3.nano", true, false, false, false, false},  // drop propagated tag
		{1, 0, "t3.nano", true, false, false, false, false},  // drop 1 az
		{0, 2, "t3.nano", true, false, false, false, false},  // add 2 subnets, drop az-s
		{0, 1, "t3.nano", true, false, false, false, false},  // drop 1 subnet
		{0, 1, "t3.nano", false, true, false, false, false},  // drop launch config, add template
		{0, 1, "t3.micro", false, true, false, false, false}, // update template
		{0, 1, "t3.micro", false, false, true, false, false}, // drop template, add mixed policy
		{0, 1, "t3.nano", false, false, true, false, false},  // update mixed policy
		{0, 1, "t3.nano", false, false, true, true, false},   // use placement group
	}
	...
	for i, test := range matrix {
		steps[i] = resource.TestStep{
			...
			Check: resource.ComposeTestCheckFunc(
				testAccCheckAWSAutoScalingGroupExists(resourceName, &group),
				checkInstanceRefreshTokenNotEquals(resourceName, &lastRefreshToken),
			),
		}
	}
...
resource "aws_default_subnet" "current" {
...
resource "aws_autoscaling_group" "test" {
  ...
  vpc_zone_identifier  = local.subnet_count > 0 ? slice(aws_default_subnet.current.*.id, 0, local.subnet_count) : null
  ...
  tag {
    key                 = "TestKey"
    value               = "TestValue"
    propagate_at_launch = local.propagate_at_launch
  }
}
`,
